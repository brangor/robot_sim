...
describe("Robot Simulation", () => {
    ...
    test("Process commands and validate outputs", async () => {
        ...
        const tempTestDataPath = path_1.default.join(__dirname, "tempTestData.json");
        await fs_1.default.promises.writeFile(tempTestDataPath, JSON.stringify(testData, null, 2));
        try {
            const { stdout, stderr } = await execAsync(`node ./dist/index.js ${tempTestDataPath}`);
            if (stderr) {
            ...
            await fs_1.default.promises.unlink(tempTestDataPath);
        }
    });
});
